...
import naxriscv.interfaces.{LockedImpl, PerformanceCounterService}
...
class MyFetchCachePlugin(var memDataWidth : Int,
                      var cacheSize: Int,
                      var wayCount: Int,
                      var refillCount : Int,
  ...
  val logic = create late new Area{
    ...
    val iCache = new DataCache(
      setup.dataCacheParameters
    )

    setup.writebackBusy <> iCache.io.writebackBusy
    setup.lockPort <> iCache.io.lock
    setup.refillEvent.map(_ := RegNext(iCache.io.refillEvent) init(False))
    setup.writebackEvent.map(_ := RegNext(iCache.io.writebackEvent) init(False))

    setup.refillCompletions := iCache.io.refillCompletions
    ...
    val load = new Area{
      ...
      iCache.io.load.cmd.valid := hit
      iCache.io.load.cmd.payload := OhMux(oh, sorted.map(_.port.cmd.payload))
      (sorted, oh.asBools).zipped.foreach(_.port.cmd.ready := _ )

      iCache.io.load.cancels := sorted.map(_.port.cancels).reduceBalancedTree(_ | _)
      iCache.io.load.translated := OhMux(ohHistory(loadTranslatedAt), sorted.map(_.port.translated))

      for((spec, sel) <- (sorted, ohHistory(loadRspAt).asBools).zipped){
        spec.port.rsp.valid := iCache.io.load.rsp.valid && sel
        spec.port.rsp.payload := iCache.io.load.rsp.payload
      }
    }

    val store = new Area{
      assert(storePorts.size == 1)
      iCache.io.store <> storePorts.head.port
    }
  }

  val mem = create late logic.iCache.io.mem.toIo()
}
